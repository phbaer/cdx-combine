...
class CDX_Combine:
    ...
    def _lookup_new_bom_ref(self, dependency: Dependency) -> str | None:
        if str(dependency.ref) not in self._new_bom_ref_map:
            self._logger.error("BOM reference %s unknown!", dependency.ref)
            return None
    ...
    def _lookup_component(self, dependency: Dependency) -> Component:
        ...
        if new_bom_ref not in self._new_bom_components:
            self._logger.error("BOM reference %s not found!", new_bom_ref)
            return None
    ...
    def _simplify_ref(self, bom_ref: str | BomRef) -> str:
        ...
        if "|" in bom_ref_str:
            ...
            self._logger.debug("Mapping %s onto %s", old_bom_ref_str, bom_ref_str)
        return bom_ref_str
    ...
    def _add_components(self, new_bom: Bom, component: Component):
        ...
        if bom_ref_str not in self._new_bom_components:
            self._logger.debug("Adding component %s", bom_ref_str)
    ...
    def _mirror_dependencies(self, new_bom: Bom, dependency: Dependency):
        self._logger.debug("Processing dependecy %s", dependency.ref)
        ...
        if dependency_component is not None:
            for subdependency in dependency.dependencies:
                ...
                if subdependency_component is not None:
                    new_bom.register_dependency(
                        dependency_component, [subdependency_component]
                    )
    ...
    def run(self, args):
        ...
        self._logger.info(
            "Generating SBOM for %s %s from %s...", name, version, ", ".join(files)
        )
        ...
        new_bom = Bom()
        new_bom.metadata.component = root_component = Component(
            name=name,
        ...
        for bom in boms:
            ...
            self._logger.info("Processing %s...", bom_ref_str_orig)
            ...
            self._new_bom_ref_map[bom_ref_str_orig] = self._get_ref_from_component(
                bom_component
            )
        ...
        self._logger.info(
            "New SBOM %s contains %d components",
            args.output,
            len(self._new_bom_components),
        )
        ...
        try:
            ...
            if validation_errors:
                self._logger.error(
                    "JSON invalid:\nValidationError:\n%s", repr(validation_errors)
                )
                # sys.exit(2)
            # print('JSON valid')
        except MissingOptionalDependencyException as error:
            self._logger.error("JSON-validation was skipped due to %s", error)
